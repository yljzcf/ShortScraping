...
// 固定卡集：转义（引号/换行/逗号）、tags/genres | 连接、imdbId 旧字段、同 itemId 去重、无键跳过
const FIXTURE = [
...
check('T1 与旧实现产物字节级一致', content === golden,
...
// 分支断言
check('T3 BOM + CRLF + 16 列表头', content.startsWith('﻿') && content.includes('\r\n')
  && content.split('\r\n')[0].replace('﻿', '') === TimelineCsv.CSV_COLUMNS.join(',')
  && TimelineCsv.CSV_COLUMNS.length === 16, '');
check('T4 引号转义与换行折空格', content.includes('""Quoted""') && content.includes('"line one line two"'), '');
check('T5 tags 竖线连接', content.includes('"IMDB|demo"'), '');
check('T6 imdbId 旧字段兼容映射到 itemId 列', content.includes('"tt0002"'), '');
check('T7 normalizeDrama 白名单 16 字段', Object.keys(TimelineCsv.normalizeDrama({})).length === 16, '');
check('T8 空表只有表头', TimelineCsv.buildTimelineCsv([]).content.trim().split('\r\n').length === 1, '');
check('T9 genres 竖线连接与缺省空列', content.includes('"Romance|Billionaire/CEO"')
  && content.split('\r\n')[2].endsWith(',""'), '');
